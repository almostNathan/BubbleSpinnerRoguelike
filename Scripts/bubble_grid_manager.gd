...
@onready var scoring_timer: Timer = $ScoringTimer
@onready var start_point : StartPoint = $StartPoint
@onready var island_check_timer: Timer = $IslandCheckTimer
...
func bubble_shot(_shot_bubble):
	print("bubble_grid, bubble shot")
	spawn_countdown -= 1
	shot_count += 1
...
func bubble_collided(shot_bubble: BaseBubble, collided_bubble: BaseBubble):
	...
	#Scoring section has been moved to score_and_clear_all
...
func rotate_bubble_grid(shot_bubble : BaseBubble, _collided_bubble : BaseBubble):
	#TODO: NEEDS some work. Rotation looks wierd in some situations with bounce MOD
	var direction_to_center : Vector2 = shot_bubble.global_position.direction_to(self.position)
	var distance_to_center : float = shot_bubble.global_position.distance_to(start_point.global_position)
	var force_angle : float = direction_to_center.angle_to(shot_bubble.movement_direction)
	var force_value : float = shot_bubble.speed / Util.base_values['base_bubble_speed'] * shot_bubble.weight / (distance_to_center / 200)
...
func score_and_clear(closest_position : Vector2i) -> void:
	...
	for bubble_pos in connected_group_pos:
		grid_slot_dict[bubble_pos].score_slot(score_number)
...
func score_and_clear_all():
	var taken_bubble_slot_pos_array = get_unavailable_slots()
	for taken_bubble_slot_pos in taken_bubble_slot_pos_array:
		if get_connected_group_pos(taken_bubble_slot_pos) != []:
			score_and_clear(taken_bubble_slot_pos)
			update_available_positions()
			
			score_and_clear_all()
			break

func _on_island_check_timer_timeout() -> void:
	delete_islands()
